...
def test_multiprocessed_vs_serial_rendering():
    """
    This function tests the rendering of Super Synaptic Objects (SSOs) using both multiprocessing and serial
    rendering methods. It checks if the views generated by both methods are identical. The function uses
    the 'render_sso_coords_multiprocessing' and 'render_sso_coords' or 'render_sso_coords_index_views' 
    functions from the 'syconn.proc.rendering' module for rendering. The SSOs are initialized from a k.zip 
    file using the 'init_sso_from_kzip' function from the 'syconn.proc.ssd_assembly' module.
    """
...
def test_raw_and_index_rendering_osmesa():
    """
    This function tests the raw and index rendering of Super Synaptic Objects (SSOs) using the 'osmesa' 
    platform. It checks if the number of index views and raw views are equal. The function uses the 
    'render_sso_coords' and 'render_sso_coords_index_views' functions from the 'syconn.proc.rendering' 
    module for rendering. The SSOs are initialized from a k.zip file using the 'init_sso_from_kzip' 
    function from the 'syconn.proc.ssd_assembly' module.
    """
...
def test_raw_and_index_rendering_egl():
    """
    This function tests the raw and index rendering of Super Synaptic Objects (SSOs) using the 'egl' 
    platform. It checks if the number of index views and raw views are equal. The function uses the 
    'render_sso_coords' and 'render_sso_coords_index_views' functions from the 'syconn.proc.rendering' 
    module for rendering. The SSOs are initialized from a k.zip file using the 'init_sso_from_kzip' 
    function from the 'syconn.proc.ssd_assembly' module.
...
def test_egl_and_osmesa_swap_and_equivalence():
    """
    This function tests the equivalence of the 'egl' and 'osmesa' platforms for raw and index rendering of 
    Super Synaptic Objects (SSOs). It checks if the fraction of different vertex indices, the maximum 
    deviation of depth value, the fraction of affected pixels with high deviation, and the fraction of 
    affected pixels are within acceptable limits. The function uses the 'render_sso_coords' and 
    'render_sso_coords_index_views' functions from the 'syconn.proc.rendering' module for rendering. The 
    SSOs are initialized from a k.zip file using the 'init_sso_from_kzip' function from the 
