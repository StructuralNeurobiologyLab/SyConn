...
from syconnfs.handler.compression import LZ4Dict, MeshDict, VoxelDict, VoxelDictL, AttributeDict
from syconnfs.handler.basics import write_txt2kzip,write_data2kzip , read_txt_from_zip, remove_from_zip
from syconnfs.config.default_wd import wd
import os
import logging
import traceback
import zipfile
...
logging.basicConfig(filename=parent_path + '/logs/unit_tests.log', level=logging.DEBUG, filemode='w')
...
def test_created_then_blocking_LZ4Dict_for_3s_2_fail_then_one_successful():
    ...
    def create_LZ4Dict_wait_for_3s_then_close():
        ...
        pkl1 = LZ4Dict(test_p, read_only=False)
        pkl1[1] = np.ones((5, 5))
        time.sleep(3)
        pkl1.save2pkl()
    ...
    def create_fail_expected_runtime_error_at_1s(a, b, q1):
        ...
        start = time.time()
        try:
            pkl2 = LZ4Dict(test_p, read_only=True, timeout=1,
    ...
    def create_fail_expected_runtime_error_at_2s(a, b, q2):
        ...
        start = time.time()
        try:
            pkl2 = LZ4Dict(test_p, read_only=True, timeout=2)
