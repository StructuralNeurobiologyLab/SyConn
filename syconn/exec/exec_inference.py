...
def run_morphology_embedding(max_n_jobs: Optional[int] = None):
    """
    This function infers local morphology embeddings for all neuron reconstructions based on
    a triplet-loss trained cellular morphology learning network (tCMN). The point-based model 
    is trained with the pts_loader_scalar (used for cell types).
    
    Args:
        max_n_jobs: Number of parallel jobs. If not provided, it defaults to the total number 
        of GPUs available multiplied by 4.
    
    Notes:
        This function requires the following functions to be run beforehand:
        - run_create_neuron_ssd from syconn.exec.exec_init
        - run_neuron_rendering
        - run_skeleton_generation from syconn.exec.skeleton
    """
...
def run_cell_embedding(max_n_jobs: Optional[int] = None):
    """
    This function infers cell embeddings for all neuron reconstructions based on
    a triplet-loss trained cellular morphology learning network (tCMN). The 
    point-based model is trained with the pts_loader_scalar (used for cell types). 
    Multi-views functionality is not implemented.
    
    Args:
        max_n_jobs: Number of parallel jobs. If not provided, it defaults to the 
        total number of GPUs available multiplied by 4.
...
def run_celltype_prediction(max_n_jobs_gpu: Optional[int] = None):
    ...
    Args:
        max_n_jobs_gpu: Number of parallel GPU jobs. If not provided, it defaults to the total 
        number of GPUs available multiplied by 4 if batch jobs are enabled, otherwise it defaults 
        to 2.
...
def run_semsegaxoness_prediction(max_n_jobs_gpu: Optional[int] = None):
    """
    This function infers and maps the semantic segmentation of the 2D projections onto the cell 
    reconstruction mesh (ssv.label_dict('vertex')) via semseg_of_sso_nocache. It generates the 
    following skeleton attributes by semsegaxoness2skel and makes them available in 
    SuperSegmentationObject.skeleton:
    ...
    Args:
        max_n_jobs_gpu: Number of parallel GPU jobs. If not provided, it defaults to the total 
        number of GPUs available multiplied by 4 if batch jobs are enabled, otherwise it defaults to 1.
    """
    if max_n_jobs_gpu is None:
...
def run_semsegspiness_prediction(max_n_jobs_gpu: Optional[int] = None):
    """
    This function stores semantic spine labels inside 
    ssv.label_dict('vertex')['spiness].
    
    Args:
        max_n_jobs_gpu: Number of parallel GPU jobs. If not provided, it 
        defaults to the total number of GPUs available multiplied by 4 if 
        batch jobs are enabled, otherwise it defaults to 1.
    """
    if max_n_jobs_gpu is None:
        max_n_jobs_gpu = global_params.config.ngpu_total * 4 if qu.batchjob_enabled() else 1
...
def run_astrocyte_prediction_pts(max_n_jobs_gpu: Optional[int] = None):
    """
    This function predicts astrocyte and neuron supervoxels using point cloud-based convolutional networks.
    It requires the function `syconn.exec_init.init_cell_subcell_sds` to have been executed beforehand.
    
    Note: Post-processing currently requires locking. To prevent locking, an additional map-reduce step
    is required to write the final probabilities of all SVs in a "per-storage" (per chunk attribute dict) 
    fashion.
...
def run_astrocyte_prediction():
    """
    This function predicts astrocyte supervoxels based on the `img2scalar` 
    Cellular Morphology Network (CMN). It requires the functions 
    `syconn.exec_init.init_cell_subcell_sds` and `run_astrocyte_rendering` 
...
def run_astrocyte_splitting():
    """
    This function uses the pruned Region Adjacency Graph (RAG) at 
    `global_params.config.pruned_svgraph_path` (stored as edge list .bz2 file) 
    which is computed in `syconn.exec.exec_init.init_cell_subcell_sds` to split 
    astrocyte fragments from neuron reconstructions and separate those and entire 
    glial cells from the neuron supervoxel graph.
    
    It stores neuron SV graph at `syconn.handler.config.DynConfig.neuron_svgraph_path`
    which is then used by `syconn.exec.exec_init.run_create_neuron_ssd`.
    
    Todo:
        * refactor how splits are stored, currently those are stored at ssv_tmp
    
    Note: This function requires `syconn.exec_init.init_cell_subcell_sds`,
    `run_astrocyte_rendering` and `run_astrocyte_prediction` to have been executed 
    beforehand.
