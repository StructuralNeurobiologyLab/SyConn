...
def kimimaro_skelgen(cube_size, cube_offset, nb_cpus: Optional[int] = None, ssd: SuperSegmentationDataset = None,
    ...
    """
    Generates a skeleton using the Kimimaro library. The function loads a segmentation from a KnossosDataset,
    downsamples it if necessary, relabels the segmentation volume with SuperSegmentationObject IDs, and then
    skeletonizes the volume using Kimimaro. The resulting skeletons are then sparsified and returned.
    
    Args:
        cube_size: The size of the cube to be processed in mag 1 voxels.
        cube_offset: The starting point of the cube in mag 1 voxel coordinates.
        nb_cpus: The number of CPUs to be used by Kimimaro. Defaults to 1 if not specified.
        ssd: The SuperSegmentationDataset to be used. If not specified, a new one is created.
        ds: The downsampling factor. If not specified, no downsampling is performed.
        dust_threshold: The threshold for removing connected components with fewer than this number of voxels.
    
    Returns:
        A dictionary mapping cell IDs to their corresponding sparsified skeletons.
    """
    if nb_cpus is None:
        nb_cpus = 1
...
def kimimaro_mergeskels(path_list: str, cell_id: int, nb_cpus: bool = None,
    ...
    """
    Merges partial skeletons generated by Kimimaro into a single skeleton. The function loads the partial skeletons
    from the specified paths, merges them into a single skeleton, and then post-processes the skeleton to remove
    small stubs and disconnected components.
    
    Args:
        path_list: A list of paths to the locations of the partial skeletons.
        cell_id: The ID of the cell whose skeletons are to be merged.
        nb_cpus: The number of CPUs to be used in the query of cKDTree during stitching. Defaults to 1 if not specified.
        dust_threshold: The threshold for removing disconnected components smaller than this size. The size is
            measured in physical distance. [From Kimimaro postprocess docs]
        tick_threshold: The threshold for removing small branches from the main skeleton. Branches larger than the
            physical tick_threshold are preserved. [From Kimimaro postprocess docs]
    
    Returns:
        The merged and post-processed skeleton. Node coordinates are in nm.
    """
    if nb_cpus is None:
        nb_cpus = 1
...
def skelcv2nxgraph(skel: cloudvolume.Skeleton) -> nx.Graph:
    """
    Converts a cloudvolume Skeleton into a networkx Graph. The resulting graph has node attributes 'position' and
    'radius' taken from the vertices and radii of the input skeleton, respectively.
    
    Args:
        skel: The cloudvolume Skeleton to be converted.
...
def nxgraph2skelcv(g: nx.Graph, radius_key: str = 'radius') -> cloudvolume.Skeleton:
    """
    Converts a networkx Graph into a cloudvolume Skeleton. The resulting skeleton has vertices and radii taken from
    the 'position' and 'radius' attributes of the nodes of the input graph, respectively.
    
    Args:
        g: The networkx Graph to be converted.
        radius_key: The key to access the radius attribute of the nodes. Defaults to 'radius'.
    
    Returns:
        The resulting cloudvolume Skeleton.
    """
...
def sparsify_skelcv(skel: cloudvolume.Skeleton, scale: Optional[np.ndarray] = None,
    ...
    """
    Sparsifies a cloudvolume Skeleton by recursively removing nodes. Leaf and branch nodes are ignored.
    
    Args:
        skel: The cloudvolume Skeleton to be sparsified. Requires 'position' attribute.
        scale: The scale factor, equal to the physical voxel size in nm. If not specified, the global scale is used.
        angle_thresh: The threshold for the angle between adjacent edges. Nodes for which the angle is greater than
            this threshold are removed.
        max_dist_thresh: The maximum desired distance between nodes. Nodes that are further apart than this threshold
            are not removed.
        min_dist_thresh: The minimum desired distance between nodes. Nodes that are closer together than this threshold
            are removed.
    
    Returns:
        The sparsified cloudvolume Skeleton.
    """
