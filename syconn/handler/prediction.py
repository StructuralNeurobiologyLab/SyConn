...
def get_glia_model_pts() -> 'InferenceModel':
    from elektronn3.models.base import InferenceModel
    from elektronn3.models.convpoint import SegSmall
    mkwargs, loader_kwargs = get_pt_kwargs(global_params.config.mpath_glia_pts)
    m = SegSmall(1, 2, **mkwargs)
    m.load_state_dict(torch.load(global_params.config.mpath_glia_pts)['model_state_dict'])
    m = InferenceModel(m)
    m.loader_kwargs = loader_kwargs
    return m


def get_compartment_model_pts() -> 'InferenceModel':
    from elektronn3.models.base import InferenceModel
    from elektronn3.models.convpoint import SegSmall2
    mkwargs, loader_kwargs = get_pt_kwargs(global_params.config.mpath_comp_pts)
    m = SegSmall2(5, 7, **mkwargs)
    m.load_state_dict(torch.load(global_params.config.mpath_comp_pts)['model_state_dict'])
    m = InferenceModel(m)
    m.loader_kwargs = loader_kwargs
    return m


def get_celltype_model_pts() -> 'InferenceModel':
    from elektronn3.models.base import InferenceModel
    from elektronn3.models.convpoint import ModelNet40
    mkwargs, loader_kwargs = get_pt_kwargs(global_params.config.mpath_celltype_pts)
    m = ModelNet40(5, 8, **mkwargs)
    m.load_state_dict(torch.load(global_params.config.mpath_celltype_pts)['model_state_dict'])
    m = InferenceModel(m)
    m.loader_kwargs = loader_kwargs
    return m


def get_tnet_model_pts() -> 'InferenceModel':
    from elektronn3.models.base import InferenceModel
    from elektronn3.models.convpoint import ModelNet40
    mkwargs, loader_kwargs = get_pt_kwargs(global_params.config.mpath_tnet_pts)
    m = ModelNet40(5, 10, **mkwargs)
    m.load_state_dict(torch.load(global_params.config.mpath_tnet_pts)['model_state_dict'])
    m = InferenceModel(global_params.config.mpath_tnet_pts)
    m.loader_kwargs = loader_kwargs
    return m
