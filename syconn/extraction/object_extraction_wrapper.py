...
def from_probabilities_to_objects(cset, filename, hdf5names, object_names=None,
    ...
    transf_func_kd_overlay : callable
        ...
        is True.
    log : logging.logger
    target_kd : KnossosDataset where the segmentation is written to.
    target_mags: Mags in which the segmentation will be exported.


    """
    assert suffix == '', 'Suffixes are not supported anymore.'
    if log is None:
        log = log_extraction
    all_times = []
    step_names = []
    ...
    if target_kd is not None:
        ...
        oes.export_cset_to_kd_batchjob({hdf5names[0]: target_kd.conf_path},
            ...
            n_max_co_processes=n_max_co_processes, target_mags=target_mags)
        all_times.append(time.time() - time_start)
        step_names.append("export KD")
    # --------------------------------------------------------------------------
        time_start = time.time()
        ...
        oes.extract_voxels(cset, filename, hdf5names, chunk_list=chunk_list, suffix=suffix,
                           ...
                           n_max_co_processes=n_max_co_processes, dataset_names=object_names)
        all_times.append(time.time() - time_start)
