...
def _collect_properties_from_ssv_partners_thread(args):
    ...
    for ssv_id in ssv_ids:  # Iterate over cells
        ...
        celltypes = [ssv_o.attr_dict['celltype_cnn']] * len(ssv_synids)
        list_ax_lat = ssv_o.attr_for_coords(ssv_syncoords, attr_keys=['axoness_avg10000', 'latent_morph'])
        curr_ax = [item[0] for item in list_ax_lat]
        curr_latent = [item[1] for item in list_ax_lat]

        curr_sp = ssv_o.semseg_for_coords(ssv_syncoords, 'spiness')

        cache_dc['ssv_synids'] = np.array(ssv_synids)
        ...
        cache_dc['curr_latent'] = np.array(curr_latent)
    ...
    for so_dir_path in so_dir_paths:
        ...
        for synssv_id in this_attr_dc.keys():
            ...
            for ssv_partner_id in synssv_o.attr_dict["neuron_partners"]:
                ...
                ssv_o.load_attr_dict()
                cache_dc = CompressedStorage(ssv_o.ssv_dir + "/cache_syn.pkl")
                # print("cache_dc['ssv_synids']= ", cache_dc['ssv_synids'])
                # print("cache_dc['partner_axoness']= ", cache_dc['partner_axoness'])
                index = np.argwhere(cache_dc['ssv_synids'] == synssv_id)
                ...
                curr_latent = cache_dc['curr_latent'][index]
