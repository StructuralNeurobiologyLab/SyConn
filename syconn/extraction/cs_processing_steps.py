...
def collect_properties_from_ssv_partners(wd, obj_version=None, ssd_version=None,
    ...
    print("to czego szukam= ", sd_syn_ssv.so_dir_paths[0])
    # if (qsub_pe is None and qsub_queue is None) or not qu.batchjob_enabled():
    #     _collect_properties_from_ssv_partners_thread(wd, obj_version, ssd_version)
    #
    # elif qu.batchjob_enabled():
    #     sd_syn_ssv = segmentation.SegmentationDataset("syn_ssv", working_dir=wd,
    #                                                    version=obj_version)
    ...
    #         multi_params.append([so_dir_paths, wd, obj_version,ssd_version])
    #
    #     _ = qu.QSUB_script(
...
def _collect_properties_from_ssv_partners_thread(*args):
    ...
    """
    # wd, obj_version, ssd_version = args
    #
    ...
    #
    # for ssv_id in np.unique(syn_neuronpartners[:, 0]):  # Iterate over cells
    #     ssv_o = ssd.get_super_segmentation_object(ssv_id)
    #     ssv_o.load_attr_dict()
    #     celltypes = []
    #     celltypes.append(ssv_o.attr_dict['celltype_cnn'])
    #
    #     ssv_synids = sd_syn_ssv.ids[sd_syn_ssv.load_cached_data("neuron_partners")[:, 0] == ssv_id]
    #     for synssv_id in ssv_synids:  # Iterate over synapses
    #         this_attr_dc = AttributeDict(so_dir_path + "/attr_dict.pkl",
    #                                      read_only=False)
    #         synssv_o = sd_syn_ssv.get_segmentation_object(synssv_id)
    #         synssv_o.load_attr_dict()
    ...
    #         curr_sp = []
    #
    #         curr_ax.append(sv_o.axoness_for_coords([synssv_o.rep_coord],
    #                                            pred_type='axoness_avg10000')[0])
    #         curr_sp.append(ssv_o.semseg_for_coords([synssv_o.rep_coord],
    #                                           'spiness'))
    #
    #         synssv_o.attr_dict.update({'partner_axoness': curr_ax, 'partner_spiness': curr_sp,
    #                                    'partner_celltypes': celltypes})
    #         this_attr_dc[synssv_id] = synssv_o.attr_dict
    #         this_attr_dc.push()
    #
    # for ssv_id in np.unique(syn_neuronpartners[:, 1]):  # Iterate over cells second partner
    #     ssv_o = ssd.get_super_segmentation_object(ssv_id)
    #     ssv_o.load_attr_dict()
    #     celltypes = ssv_o.attr_dict['celltype_cnn']
    #
    #     ssv_synids = sd_syn_ssv.ids[sd_syn_ssv.load_cached_data("neuron_partners")[:, 0] == ssv_id]
    #     for synssv_id in ssv_synids:  # Iterate over synapses
    #
    ...
    #         synssv_o.load_attr_dict()
    #
    #         curr_ax = sv_o.axoness_for_coords([synssv_o.rep_coord],
    #                                            pred_type='axoness_avg10000')[0]
    #         curr_sp = ssv_o.semseg_for_coords([synssv_o.rep_coord], 'spiness')
    #
    #         sym_asym_ratio = synssv_o.attr_dict['syn_type_sym_ratio']
    #         syn_sign = -1 if sym_asym_ratio > global_params.sym_thresh else 1
    #         synssv_o.attr_dict.update({'partner_axoness': synssv_o.attr_dict[partner_axoness].append(curr_ax),
    #                                    'partner_spiness': synssv_o.attr_dict[partner_spiness].append(curr_sp),
    #                                    'partner_celltypes': synssv_o.attr_dict[partner_spiness].append(curr_sp),
    #                                    'syn_sign': syn_sign})
    #         this_attr_dc[synssv_id] = synssv_o.attr_dict
    #         this_attr_dc.push()
    #
