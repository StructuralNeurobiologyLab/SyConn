...
cdef uint64_t[:, :, :] process_block(uint32_t[:, :, :] edges, uint32_t[:, :, :] arr, stencil1=(7,7,
3)):
    sh = arr.shape
    cdef int stencil[3]
    ...
    cdef uint64_t[:, :, ::1] out = cvarray(shape = (arr.shape[0], arr.shape[1],
    ...
    for x in range(offset[0], sh[0] - offset[0]):
        for y in range(offset[1], sh[1] - offset[1]):
            for z in range(offset[2], sh[2] - offset[2]):
                if edges[x, y, z] == 0:
                    continue
...
def process_block_nonzero(uint32_t[:, :, :] edges, uint32_t[:, :, :] arr, stencil1=(7,7,3)):
    ...
    for x in range(0, sh[0]-2*offset[0]):
        for y in range(0, sh[1]-2*offset[1]):
            for z in range(0, sh[2]-2*offset[2]):
