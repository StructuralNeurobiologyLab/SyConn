...
class SyConnBackend(object):
    ...
    def ssv_mesh(self, ssv_id):
        ...
        mesh = {'num_vertices': len(mesh[1]),        #number of vertices
    ...
    def cts_in_data(self, gt_type):
        """
        Returns the available cell types based on the dataset's ground truth type
        :param gt_type: str
        :return: tuple
        """
        ...
        switch = {
            "ctgt": ("MSN", "EA", "GP", "INT"),
            "ctgt_v2": ("STN", "modulatory", "MSN", "LMAN", "HVC", "GP", "INT"),
            "ctgt_j0251": ("STN", "DA", "MSN", "LMAN", "HVC", "TAN", "GPe", "GPi", "FS", "LTS"),
            "ctgt_j0251_v2": ("STN", "DA", "MSN", "LMAN", "HVC", "TAN", "GPe", "GPi", "FS", "LTS", "NGF")
        }

        return switch.get(gt_type, "Invalid argument passed gt_type.")

    def mito_mappings(self):
        mi_mappings = self.ssd.load_numpy_data('mi')
        # print(mi_mappings.shape)
