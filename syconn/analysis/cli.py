...
from syconn.analysis.flask_server import createDownloadUrl
import json
import argparse
import os
import numpy as np
...
def configure_viewer(backend: SyConnBackend, state, raw_dataset=None, seg_dataset=None, dimensions=None, flask_PORT=8000):
    """
    ...
    state.layers.append(
        name='mitochondria',
        layer=neuroglancer.SegmentationLayer(
            source=f'precomputed://http://127.0.0.1:{flask_PORT}/mi',
    ...
    state.layers.append(
        name='vesicle clouds',
        layer=neuroglancer.SegmentationLayer(
            source=f'precomputed://http://127.0.0.1:{flask_PORT}/vc',
    ...
    state.layers.append(
        ...
        layer=neuroglancer.SegmentationLayer(
            source=f'precomputed://http://127.0.0.1:{flask_PORT}/sj',
