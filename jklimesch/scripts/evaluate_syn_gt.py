...
if __name__ == "__main__":
    ...
    for key in data:
        if '_l' not in key:
            ...
            pc_ads.save2pkl(os.path.expanduser(f'~/thesis/current_work/paper/syn_tests/{sso_id}_ads.pkl'))
            pc_dnh.save2pkl(os.path.expanduser(f'~/thesis/current_work/paper/syn_tests/{sso_id}_dnh.pkl'))
            ...
            for ix in range(len(coords)):
                ...
                if result[ix] != gt[ix]:
                    ...
                    pc_local.save2pkl(
                        os.path.expanduser(f'~/thesis/current_work/paper/syn_tests/examples/{sso_id}_{ix}_p{int(result[ix])}_gt{int(gt[ix])}.pkl'))
