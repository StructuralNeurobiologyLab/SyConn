    ...
    for radius in radii:
        for (optimizer, scheduler) in optimizers:
            for reg in regression:
                params.append(
                    [cnn_script,
                     dict(sr=save_root, r=radius, model=model, opt=optimizer, lr=scheduler, conv=conv, sp=npoints, ctx=ctx,
                          scale_norm=scale, use_bias=True, reg=reg)])
    params = list(basics.chunkify_successive(params, 1))
    batchjob_script(params, 'launch_trainer', n_cores=10,
                    additional_flags='--time=7-0 --qos=720h --mem=125000 --gres=gpu:1',
                    disable_batchjob=False,
                    job_name='mergeReg',
                    batchjob_folder=f'/wholebrain/scratch/amancu/mergeError/Nodes/BatchJobs/to_node_classifier/',
