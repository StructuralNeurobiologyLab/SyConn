...
if save_root is None:
    save_root = '~/mergeError/'
save_root = os.path.expanduser(save_root)

# CREATE NETWORK AND PREPARE DATA SET

# Model selection
model = RandLANet(input_channels, num_classes + 1, dropout=dr, use_norm=use_norm,
                  track_running_stats=track_running_stats, act=act, use_bias=use_bias)

# model = ConvPt(input_channels, num_classes + 1, dropout=dr, use_norm=use_norm,
#                   track_running_stats=track_running_stats, act=act, use_bias=use_bias)
...
train_ds = CloudFalseMergeLoader(npoints=npoints, transform=train_transform, use_subcell=use_subcell,
                           batch_size=batch_size, ctx_size=ctx, mask_boarders_with_id=num_classes)
valid_ds = CloudFalseMergeLoader(npoints=npoints, transform=valid_transform, train=False, use_subcell=use_subcell,
                           batch_size=batch_size, ctx_size=ctx, mask_boarders_with_id=num_classes)
