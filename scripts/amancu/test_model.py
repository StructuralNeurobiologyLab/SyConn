...
from filterLoader import DeterministicLoader
...
parser.add_argument('--lr', type=str, default='StepLR',
                    help='Chosen learning rate: StepLR/ExponentialLR/CyclicLR/ConstantLR')
...
parser.add_argument('--ctx', default=40000, help='Context size in nm', type=float)
parser.add_argument(
...
if save_root is None:
    ...
    save_root = '/wholebrain/scratch/amancu/mergeError/trainings/test/see/'
...
if modelselect == 'lcp':
    ...
    name += '_architecture_large_augmentations_gn'
...
train_ds = DeterministicLoader(radius=radius, npoints=npoints, transform=train_transform,
                               batch_size=batch_size, ctx_size=ctx)
valid_ds = DeterministicLoader(radius=radius, npoints=npoints, transform=valid_transform, train=False,
                               batch_size=batch_size, ctx_size=ctx)
