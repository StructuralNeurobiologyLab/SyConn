...
from syconn.handler.basics import write_obj2pkl, kd_factory
...
if __name__ == "__main__":
    ssd_new = SuperSegmentationDataset(
        working_dir="/wholebrain/songbird/j0126/areaxfs_v6/", version="gliagt")
    kd_seg = kd_factory(global_params.config.kd_seg_path)
    # retrieve majority SV ID at all locations
    r = 5
    glia_ids = []
    for c in np.array(glia_coords):
        seg = kd_seg.load_seg(offset=c-r, size=[2*r+1, 2*r+1, r], mag=1).swapaxes(0, 2).flatten()
        seg = seg[seg != 0]
        cnt = Counter(seg)
        glia_ids.append(cnt.most_common(1)[0][0])
    if 0 in glia_ids:
        raise ValueError
    for k in glia_ids: ssd_new.mapping_dict[k] = [k]
    # save SSVs
    ssd_new.save_dataset_deep(use_batchjob=False)
