...
if __name__ == '__main__':
    ...
    perf_res_dc = collections.defaultdict(list)
    for run in range(n_runs):
        ...
        for CV in range(ncv_min, n_cv):
            ...
            if overwrite or not os.path.isfile(fname_pred):
                res_dc = predict_celltype_gt(ssd_kwargs, mpath=mpath, redundancy=(25, 100), bs=10, nloader=10, device='cuda',
    ...
    df = pandas.DataFrame(data={'quantity': np.concatenate([[int2str_label[ii]] * n_runs for ii in range(nclasses)] +
                                ...
                                'f1score': fscores})
    create_catplot(f"{base_dir}/performances.png", qs=df, x='quantity', y='f1score', size=10)
    ...
    create_catplot(f"{base_dir}/certainty.png", qs=df, x='quantity', y='certainty', add_boxplot=True, size=4)
